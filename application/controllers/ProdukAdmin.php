...
class ProdukAdmin extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view('produk/produkadmin');
		$this->load->view('footerAdmin');
	}
	public function searchdataproduk()
	{
		$begin = $this->input->post('begin');
		$end = $this->input->post('end');

		$data = $this->ProdukModel->getListProduk();
	...
	{
		$data['kategori'] = json_encode($this->ProdukModel->getListKategori());
		$data['size'] = json_encode($this->ProdukModel->getListSize());
		$data['color'] = json_encode($this->ProdukModel->getListColor());
		$data['kategori1'] = $this->ProdukModel->getListKategori();
		$data['size1'] = $this->ProdukModel->getListSize();
		$data['color1'] = $this->ProdukModel->getListColor();

		$data['detailProduct'] = $this->ProdukModel->getListDetailProduct($id);

		$data['data'] = $this->ProdukModel->getdetailProduk($id);
		$this->load->view('headerAdmin');
